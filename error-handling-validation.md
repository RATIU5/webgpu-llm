...
const buffer1 = device.createBuffer({ size: 256, usage: GPUBufferUsage.UNIFORM });
const buffer2 = device.createBuffer({ size: 512, usage: GPUBufferUsage.STORAGE });
...
class WebGPUApplication {
  ...
  handleUncapturedError(error) {
    ...
    if (error instanceof GPUValidationError) {
      this.showUserMessage('A graphics error occurred. Please refresh the page.');
    } else if (error instanceof GPUOutOfMemoryError) {
      this.showUserMessage('Insufficient GPU memory. Try closing other applications.');
    } else if (error instanceof GPUInternalError) {
      this.showUserMessage('A GPU error occurred. Please update your graphics drivers.');
    }
...
class ResilientWebGPUApp {
  ...
  async handleDeviceLost(info) {
    ...
    try {
      ...
      this.showFatalError('GPU device could not be recovered. Please refresh the page.');
    }
  }
...
class WebGPURenderer {
  ...
  renderWithCanvas2D(ctx) {
    ...
    ctx.fillText('GPU rendering unavailable', this.canvas.width / 2, this.canvas.height / 2);
  }
...
const rawBuffer = device.createBuffer({
  ...
  usage: GPUBufferUsage.STORAGE |
         ...
         GPUBufferUsage.COPY_SRC // Must manually specify all needed flags
});
```
...
class DebugGPUDevice {
  ...
  submit(commandBuffers) {
    const opId = ++this.operationCount;
    console.log(`[${opId}] Submitting ${commandBuffers.length} command buffers`);
...
class ProductionGPUApp {
  async initialize() {
    ...
    this.device.addEventListener('uncapturederror', (event) => {
      ...
      // Show user message
      this.showErrorNotification('A graphics error occurred. Please try refreshing.');
    });
...
function showGPUError(error) {
  ...
  if (error instanceof GPUValidationError) {
    userMessage = 'The application encountered a graphics configuration error. Please refresh the page.';
  } else if (error instanceof GPUOutOfMemoryError) {
    userMessage = 'Insufficient GPU memory. Try closing other applications or reducing graphics quality.';
  } else if (error instanceof GPUInternalError) {
    userMessage = 'A GPU driver error occurred. Please update your graphics drivers and try again.';
  }
...
async function createBufferSafely(device, size, data) {
  ...
  if (memError || valError) {
    throw new Error(`Buffer creation failed: ${memError?.message || valError?.message}`);
  }
