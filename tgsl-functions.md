...
const updateParticle = tgpu
  ...
  .does((particles, index) => {
    ...
    particles[index].position = particles[index].position.add(particles[index].velocity);
  });
```
...
const positions = root
  .createBuffer(d.arrayOf(d.vec3f, 100))
  .$usage('storage');
...
```typescript
const clampValue = tgpu.fn([d.f32, d.f32, d.f32], d.f32).does((val, min, max) => {
  'use gpu';
...
const shader = tgpu.fn([d.vec3f], d.f32).does((v) => {
  ...
  let sum = 0;
  for (let i = 0; i < 3; i++) {  // Constant bound
    sum += v[i];
  }
...
const slowPipeline = root
  .makeComputePipeline(shader)
  .$workgroupSize(37);  // Poor GPU utilization

// GOOD: Power-of-2 workgroup size
const fastPipeline = root
  .makeComputePipeline(shader)
  .$workgroupSize(64);  // Better GPU utilization
