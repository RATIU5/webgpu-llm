# Slots and Derived Values in TypeGPU
...
const animate = tgpu.fn([], d.vec3f)((pos) => {
  'use gpu';
...
const sampleTexture = tgpu
  .fn([d.vec2f], d.vec4f)
  .$uses({ tex: textureSlot, smp: samplerSlot })
  ((uv) => {
    'use gpu';
...
for (let material of uniqueMaterials) {
  ...
  for (let object of objectsWithMaterial[material]) {
    pipeline
      .with(transformSlot, object.transform)
      .draw(object.vertexCount);
  }
}
```
...
for (let obj of objects) {
  if (obj.material !== currentMaterial) {
    pipeline
      .with(materialSlot, obj.material);
    currentMaterial = obj.material;
  }

  pipeline
    .with(transformSlot, obj.transform)
    .draw(obj.vertexCount);
}
```
...
const fragmentFn = tgpu
  .fn([d.vec2f], d.vec4f)
  .$uses({ time: timeSlot, color: colorSlot })
  ((uv) => {
    'use gpu';
