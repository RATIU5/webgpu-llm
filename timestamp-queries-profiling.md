...
const supportedOptionalFeatures = optionalFeatures.filter(
...
commandEncoder.copyBufferToBuffer(
  queryBuffer, 0,    // Source
  resultBuffer, 0,   // Destination
  2 * 8              // Size in bytes
);
```
...
const geometryPass = geometryPipeline
  .withTimestampWrites({
    querySet,
...
const lightingPass = lightingPipeline
  .withTimestampWrites({
    querySet,
...
const postPass = postProcessPipeline
  .withTimestampWrites({
    querySet,
...
```typescript
const ENABLE_PROFILING = import.meta.env.DEV || localStorage.getItem('enable-gpu-profiling');
