...
const vertices = new Float32Array([
  // x,    y,     r,    g,    b,    a
  -0.5, -0.5, 1.0, 0.0, 0.0, 1.0, 0.5, -0.5, 0.0, 1.0, 0.0, 1.0, 0.0, 0.5, 0.0,
  0.0, 1.0, 1.0,
]);
...
const indices = new Uint32Array([
  ...
  2,
  1,
  3, // second triangle (reuses vertices 1 and 2)
]);
...
const buffer = device.createBuffer({
  ...
  usage:
    GPUBufferUsage.STORAGE | // usage flags (required)
...
// Structured data
const particleBuffer = root.createBuffer(
  d.struct({
    position: d.vec3f,
    velocity: d.vec3f,
    health: d.f32,
...
```typescript
const transformBuffer = root.createBuffer(
  d.struct({
    scale: d.vec3f,
...
```typescript
const fixedBuffer = root.createBuffer(d.arrayOf(d.f32, 1000)).$usage("storage");
...
```typescript
const sceneBuffer = root.createBuffer(
  d.struct({
    lights: d.arrayOf(
      d.struct({
        position: d.vec3f,
...
sceneBuffer.write({
  lights: [
    ...
    {
      position: d.vec3f(-10, 20, 30),
      color: d.vec3f(1, 0.8, 0.6),
      intensity: 0.8,
    },
    // ... more lights
  ],
...
downloadEncoder.copyBufferToBuffer(
  gpuBuffer,
  0,
  downloadStagingBuffer,
  0,
  4096,
);
...
const results = new Float32Array(
  downloadStagingBuffer.getMappedRange(),
).slice();
...
const data = new Float32Array([
  ...
  0,
  0,
  0, // vec3 color (4th element is padding)
  0.5,
  0,
  0,
  0, // f32 intensity (3 padding floats)
]);
```
