...
async function main() {
  ...
  const outputBuffer = root.createBuffer(arrayOf(f32, 5)).$usage("storage");
  ...
  const pipeline = root.makeComputePipeline(squareNumbers).$workgroupSize(1); // One thread per workgroup
...
```typescript
const buffer = root.createBuffer(schema).$usage("storage").$usage("copy-src"); // Can be used in shaders AND read back to CPU
```
...
_This documentation is based on TypeGPU version 0.8.2. For the latest updates and features, always refer to the [official documentation](https://docs.swmansion.com/TypeGPU/)._
