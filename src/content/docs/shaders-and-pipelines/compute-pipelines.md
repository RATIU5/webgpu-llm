...
passEncoder.setPipeline(computePipeline);
passEncoder.setBindGroup(0, bindGroup);
passEncoder.dispatchWorkgroups(Math.ceil(elementCount / workgroupSize));
...
const passEncoder = commandEncoder.beginComputePass({
  ...
  timestampWrites: {
    // Optional: GPU timestamps for profiling
...
```typescript
// Dispatch work
pipeline.with(bindGroup).dispatchWorkgroups(workgroupCount);
...
if (requestedSize > maxWorkgroupSize) {
  throw new Error(
    `Workgroup size ${requestedSize} exceeds limit ${maxWorkgroupSize}`,
  );
}
```
