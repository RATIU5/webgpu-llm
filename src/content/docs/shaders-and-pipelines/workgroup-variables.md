...
const parallelSumKernel = tgsl.workgroupSize(256).kernel([], () => {
...
// Create kernel
const reduceKernel = tgsl.workgroupSize(256).kernel([], () => {
  const localIdx = builtin.localInvocationIndex;
...
root.execute(
  reduceKernel.with({ input, output }).dispatch([4]), // 4 workgroups of 256 threads each
);
```
