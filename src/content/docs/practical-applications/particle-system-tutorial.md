---
title: Building a Particle System with TypeGPU
sidebar:
  order: 10
---
        ...
        .else(() => {
          ...
          tgpu.if(params.mouseActive > 0, () => {
            ...
            tgpu.if(dist > 0.01 && dist < 1.5, () => {
              const force =
                (toMouse / dist) * params.mouseForce * params.deltaTime;
              vel = vel + force;
            });
          });
          ...
          tgpu.if(tgpu.abs(pos.x) > params.bounds.x, () => {
            pos = tgpu.vec3f(tgpu.sign(pos.x) * params.bounds.x, pos.y, pos.z);
            vel = tgpu.vec3f(-vel.x * 0.8, vel.y, vel.z);
          });

          tgpu.if(tgpu.abs(pos.y) > params.bounds.y, () => {
            pos = tgpu.vec3f(pos.x, tgpu.sign(pos.y) * params.bounds.y, pos.z);
...
export function createRenderPipeline(
  ...
  const fragmentShader = tgpu
    ...
    .implement((color, age, pointCoord) => {
      ...
      return {
        color: tgpu.vec4f(color.r, color.g, color.b, color.a * edge),
      };
    });
...
async function main() {
  ...
  function frame() {
    ...
    const renderBindGroup = device.createBindGroup({
      ...
      entries: [{ binding: 0, resource: { buffer: getOutputBuffer() } }],
    });
